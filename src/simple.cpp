...
public:
    ...
    Runner(ARGS... args) : app(std::make_unique<APP>(args...))
    {
    }
    void start()
    {
...
public:
    template <class... Ts>
    Application(Ts... ts) : System::StepperApplication(ts...) {}
    bool step() override
    {
        {
            std::lock_guard<std::mutex> lock(cqMutex);
...
template <typename T>
struct Validator : public crtp<T>, public BaseValidator
{
    template <class... Ts>
    Validator(Ts... ts) : BaseValidator(ts...) {}
...
struct IsEvenChecker : public Validator<IsEvenChecker>
{
    ...
    template <class... Ts>
    IsEvenChecker(Ts... ts) : Validator<IsEvenChecker>(ts...) {}
...
struct IsOddChecker : public Validator<IsOddChecker>
{
    using result_type = bool;
    template <class... Ts>
    IsOddChecker(Ts... ts) : Validator<IsOddChecker>(ts...) {}
...
struct IsPrimeChecker : public Validator<IsPrimeChecker>
{
    using result_type = bool;
    template <class... Ts>
    IsPrimeChecker(Ts... ts) : Validator<IsPrimeChecker>(ts...) {}
...
public:
    ...
    Application(Ts... ts, const Validators &v, const OPController &opi)
        : System::StepperApplication(ts...), validators(v), op_arranger(opi)
    {
    }
...
auto CreateOutputArranger()
{
    ...
    using DerivedInfo = std::tuple<
        IsEvenChecker::result_type, IsOddChecker::result_type, IsPrimeChecker::result_type>;
...
auto CreateValidators()
{
    ...
    Validators validators{
        std::make_shared<IsEvenChecker>("IsEvenChecker"), std::make_shared<IsOddChecker>("IsOddChecker"), std::make_shared<IsPrimeChecker>("IsPrimeChecker")};
    return validators;
}
